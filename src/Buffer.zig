...
pub fn create(bar: *Bar) *Self {
    ...
    const fd = posix.memfd_create("bridge-shm-buffer", std.os.linux.MFD.CLOEXEC) catch unreachable;
    defer posix.close(fd);
    posix.ftruncate(fd, @intCast(size)) catch unreachable;
...
pub fn stamp(self: *Self, key: [*:0]const u8, position: i32, background: ?*const pixman.Color, foreground: *const pixman.Color) i32 {
    const icon = self.bar.getIcon(key);
    ...
    for (icon.run.glyphs, 0..icon.run.count) |glyph, _| {
