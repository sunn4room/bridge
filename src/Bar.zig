    ...
    if (self.dirty) {
...

        {
            ...
            while (window_iterator.next()) |window| {
                if (window.available and window.placed == self.output) {
                    ...
                    if (window.visible) {
                        weights_width += self.getIcon(config.box_icons[window.weight - 1]).width;
                    }
                }
            }
        }

        {
            ...
            while (index <= 10) : (index += 1) {
                ...
                const icon = config.boxes_icons[index - 1];
                ...
                if (has_focused) {
                    ...
                    if (has_window) {
                        views_position = buffer.stamp(icon, views_position, null, &config.bar_foreground);
                    }
                }
...

        {
            ...
            while (window_iterator.next()) |window| {
                if (window.available and window.placed == self.output) {
                    ...
                    if (window.focused != 0) {
                        if (window.sticky) {
                            windows_position = buffer.stamp(window_icon, windows_position, &config.bar_foreground, &config.bar_selection);
                            weights_position = buffer.stamp(weight_icon, weights_position, &config.bar_selection, &config.bar_foreground);
                        } else {
                            windows_position = buffer.stamp(window_icon, windows_position, &config.bar_theme, &config.bar_selection);
                            weights_position = buffer.stamp(weight_icon, weights_position, &config.bar_selection, &config.bar_theme);
                        }
                    } else {
                        if (window.sticky) {
                            windows_position = buffer.stamp(window_icon, windows_position, &config.bar_selection, &config.bar_foreground);
                            weights_position = buffer.stamp(weight_icon, weights_position, null, &config.bar_foreground);
                        } else {
                            windows_position = buffer.stamp(window_icon, windows_position, null, &config.bar_foreground);
                        }
                    }
                    window.buttons = .{
